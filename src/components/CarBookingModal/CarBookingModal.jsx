...
import styles from './CarBookingModal.module.scss'
...
export default function CarBookingModal({ open, car, onClose, onConfirm }) {
      ...
      {step === 1 && (
        <>
          ...
          <DialogContent className={styles.content}>
            {/* Car header */}
            <Box className={styles.carHeader}>
              <DirectionsCarIcon className={styles.carIcon} />
              <Box>
                <Typography className={styles.carName}>{car.make} {car.model} {car.year}</Typography>
                <Typography className={styles.carCategory}>{car.category}</Typography>
              </Box>
            </Box>
            ...
            <Box className={styles.infoGrid}>
              <Box className={styles.infoItem}>
                ...
                <Box>
                  <Typography className={styles.label}>Seats</Typography>
                  <Typography className={styles.value}>{car.seats}</Typography>
                </Box>
              </Box>
              <Box className={styles.infoItem}>
                <SettingsIcon className={styles.infoIcon} />
                <Box>
                  <Typography className={styles.label}>Transmission</Typography>
                  <Typography className={styles.value} sx={{ textTransform: 'capitalize' }}>{car.transmission}</Typography>
                </Box>
              </Box>
              {car.fuelType && (
                <Box className={styles.infoItem}>
                  <Box>
                    <Typography className={styles.label}>Fuel type</Typography>
                    <Typography className={styles.value} sx={{ textTransform: 'capitalize' }}>{car.fuelType}</Typography>
                  </Box>
                </Box>
              )}
              <Box className={styles.infoItem}>
                ...
                <Box>
                  <Typography className={styles.label}>Pick-up location</Typography>
                  <Typography className={styles.value}>{car.city}, {car.country}</Typography>
                </Box>
              </Box>
            </Box>

            <Box className={styles.priceRow}>
              <Typography className={styles.priceLabel}>Price per day</Typography>
              <Typography className={styles.price}>€{car.pricePerDay}</Typography>
            </Box>
          </DialogContent>

          <DialogActions className={styles.actions}>
            <Button onClick={handleClose} className={styles.cancelBtn}>Cancel</Button>
            <Button variant="contained" onClick={() => setStep(2)} className={styles.primaryBtn}>
              Continue to Payment
            </Button>
          </DialogActions>
        </>
      ...
      {step === 2 && (
        <>
          ...
          <DialogActions className={styles.actions}>
            ...
            >
              {loading ? 'Processing…' : `Pay €${car.pricePerDay}/day`}
            </Button>
          </DialogActions>
        </>
      )}
